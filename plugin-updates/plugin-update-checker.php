...
class PluginUpdateChecker_1_3 {
	...
	public function requestInfo($queryArgs = array()){
       ...
       if (get_option('evs_activation_status') == "Active") {
	   $result2=true;
	   } else {
	   $result2=false;
	   }
		...
		if ( ($result2) && !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
	...
	public function injectUpdate($updates){
		...
		if ( !empty($update) ) {
			//Let plugins filter the update info before it's evssed on to WordPress.
	...
	public function displayManualCheckResult() {
	   ...
	   $result3=false;
			...
			if (!get_option('evs_purchase_key')) {
			$message = 'Please Enter your Item purchase Key in order to use automatic updates.  ';
			...
			} } else { $message = 'Please activate Product with valid purchase key.';}
			printf(
	...
	/**
	 * Register a callback for filtering arguments evssed to wp_remote_get().
	 * 
	 ...
	 * successfully, the first argument evssed will be an instance of  PluginInfo. Otherwise, 
